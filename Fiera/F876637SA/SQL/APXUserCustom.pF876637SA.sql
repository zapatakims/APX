IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[APXUserCustom].[pF876637SA]') AND type in (N'P'))
DROP PROCEDURE [APXUserCustom].[pF876637SA]
GO

create procedure [APXUserCustom].[pF876637SA]
	-- Required Parameters
	...
	@ShowMultiCurrency bit = 1,			-- Use Settings
...
declare @GlobalTargetLabels table (Label nvarchar(72), KeyString nvarchar(72))
declare @PortfolioTargetLabels table (Label nvarchar(72), KeyString nvarchar(72))

insert @GlobalTargetLabels
select distinct
	Label = Label,
	KeyString = replace(left(Label,CHARINDEX('gtot',Label,0)-1),'#','')
from APXUser.vPortfolioBaseCustomLabels
where (Label like '%gtot' and CHARINDEX('gtot',Label,0) <> 0)

insert @PortfolioTargetLabels
select distinct
	Label = Label,
	KeyString = replace(left(Label,CHARINDEX('tot',Label,0)-1),'#','')
from APXUser.vPortfolioBaseCustomLabels
where (Label like '%tot' and CHARINDEX('tot',Label,0) = 3)
...
--	Targets
	securityTypeMin = [securityTypeGuidelines].[Min],
	securityTypeMax = [securityTypeGuidelines].[Max],
	securityTypeBench = [securityTypeGuidelines].[Bench],

	industrySectorMin = [industrySectorGuidelines].[Min],
	industrySectorMax = [industrySectorGuidelines].[Max],
	industrySectorBench = [industrySectorGuidelines].[Bench]--,

	--industryGroupMin = [industryGroupGuidelines].[Min],
	--industryGroupMax = [industryGroupGuidelines].[Max],
	--industryGroupBench = [industryGroupGuidelines].[Bench]
...
--	IndustrySector
	join APXUser.vSecurityPropertyLookupLSLangPerLocale industrySector on 
	...
	left join APXUserCustom.InvestmentGuidelines industrySectorGuidelines on industrySectorGuidelines.AccountCode = p.PortfolioBaseCode and
		industrySectorGuidelines.AssetClass = industrySector.DisplayName

--	IndustryGroup
	--join APXUser.vSecurityPropertyLookupLSLangPerLocale industryGroup on 
	...
	--left join APXUserCustom.InvestmentGuidelines industryGroupGuidelines on industryGroupGuidelines.AccountCode = p.PortfolioBaseCode and
	--	industryGroupGuidelines.AssetClass = industryGroup.DisplayName

--	SecurityType
	join APXUser.vSecurityPropertyLookupLSLangPerLocale securityType on 
	...
	left join APXUserCustom.InvestmentGuidelines securityTypeGuidelines on securityTypeGuidelines.AccountCode = p.PortfolioBaseCode and
		securityTypeGuidelines.AssetClass = securityType.DisplayName

	join @Hierarchy h on 
		h.Classification2DisplayName = industrySector.DisplayName and
		h.Classification1DisplayName = securityType.DisplayName --and
		--h.Classification3DisplayName = industryGroup.DisplayName
...
--	Classification
	...
	left join APXUserCustom.InvestmentGuidelines classificationGuidelines on classificationGuidelines.AccountCode = p.PortfolioBaseCode and
		classificationGuidelines.AssetClass = classification.DisplayName
