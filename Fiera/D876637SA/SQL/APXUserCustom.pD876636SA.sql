...
/****** Object:  StoredProcedure [APXUserCustom].[pD876637SA]    Script Date: 01/04/2016 11:42:38 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[APXUserCustom].[pD876637SA]') AND type in (N'P', N'PC'))
...
EXEC [APXUserCustom].[pD876637SA]
	...
	@Portfolios = '+@HOMEMRG',
	@ToDate = '09/30/2015',
	@ClassificationID1 = 2059,
	@ClassificationID2 = 2058,
	@ClassificationID3 = 2057,
...
-- ***************  Cumulative Performance History Detail ***************
...
-- ***************  Performance History Summary for latest 2-years ***************
declare @ExcludeSinceDateIRR bit = 1
declare @FromDate datetime 
select @FromDate = APXUser.fgetgenericdate('{edtm}',DATEADD(YYYY,-2,@ToDate))
...
-- ***************  Performance History Summary for latest 4-years ***************
select @FromDate = APXUser.fgetgenericdate('{edtm}',DATEADD(YYYY,-4,@ToDate))
exec APXUser.pPerformanceHistoryPeriodBatch
	-- Required Parameters
	@DataHandle = @DataHandle,
...
-- Repeat perfhist calc for @classificationid 2
...
-- Repeat perfhist calc for @classificationid 3
...
begin
	...
	-- ***************  Cumulative Performance History -9 tot port fix for assetclass1 start dates***************
	exec APXUser.pPerformanceHistoryBatch
		-- Required Parameters
		@DataHandle = @DataHandle,
		@DataName = 'totport',
	...
	-- ***************  Performance History Summary for latest 4-year  -9 tot port fix***************
	select @FromDate = APXUser.fgetgenericdate('{edtm}',DATEADD(YYYY,-4,@ToDate))
	exec APXUser.pPerformanceHistoryPeriodBatch
		-- Required Parameters
		@DataHandle = @DataHandle,
...
-- Build @ReportData table for output
-- 1. Insert Standard parent module data
insert @reportData
	SELECT 
		DataHandle = @DataHandle,
...
-- 2. Create and populate filter table for @ClassificationID3, 
--    fileter on ClassficationName = @ClassificationName and DisplayName = @KeyString
...
-- 3. Setup to get inception to date for classifications
...
-- 4. Get Inception to Date for classifciations
...
begin
	...
	insert #InceptionDates
		select ClassificationID, ClassificationMemberID, InceptionDate 
		from @temp 
