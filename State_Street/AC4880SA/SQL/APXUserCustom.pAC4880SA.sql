...
/****** Object:  StoredProcedure [APXUserCustom].[pAC4880SA]    Script Date: 03/13/2015 18:15:58 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[APXUserCustom].[pAC4880SA]') AND type in (N'P', N'PC'))
DROP PROCEDURE [APXUserCustom].[pAC4880SA]
GO
...
CREATE procedure [APXUserCustom].[pAC4880SA]
@SessionGuid nvarchar(48),
@PortfolioSearch nvarchar(max),
@FromDate date,
@ThruDate date
...
select PortfolioBaseID,PortfolioBaseCode from APXUser.vPortfolioBase where PortfolioBaseID in (select string from APX.fSplitString(@PortfolioSearch,','))
...
--insert first row and delete it from customtable to remove the chances of duplicate row
--insert into @tempPBAudit
--select top 1 * from APXUserCustom.tAC4880SA_AoObjectAudit_Hist order by AuditEventTime desc
--delete from APXUserCustom.tAC4880SA_AoObjectAudit_Hist where AuditEventTime = (select time from @audittemp)
...
SELECT * FROM (
SELECT A.*
, COALESCE(c.ContactCode, 'Portfolio') [ContactCode], COALESCE(p.PortfolioBaseCode, '') [PortfolioBaseCode], v.AuditEventTime, d.UserBaseName, e.DisplayName [FunctionDisplayName], d.DisplayName [AuditUserName]
 FROM
(
SELECT 'Portfolio' [FieldType], p.PortfolioBaseID, 0 [ContactID], p.FieldName, COALESCE(p.OldValue,'') OldValue, COALESCE(p.NewValue,'') NewValue, p.AuditEventID FROM ApxUserCustom.tAC4880SA_Changes_PortfolioFields p
UNION
SELECT
 'Contact' [FieldType], i.PortfolioID, i.ContactID [ContactID], c.FieldName, COALESCE(c.OldValue,'') OldValue, COALESCE(c.NewValue,'') NewValue, c.AuditEventID FROM AdvApp.vPortfolioInterestedParty i 
JOIN ApxUserCustom.tAC4880SA_Changes_ContactFields c on c.ContactID = i.ContactID
) A
JOIN dbo.AdvAuditEvent v ON a.AuditEventID = v.AuditEventID
JOIN AdvApp.vUserBase d on v.UserID = d.UserBaseID
JOIN AdvApp.vFunction e on v.FunctionID = e.FunctionID
LEFT JOIN AdvApp.vPortfolioBase p on p.PortfolioBaseID = A.PortfolioBaseID  --Doing a left join because maybe some portfolios get deleted?
LEFT JOIN AdvApp.vContact c on c.ContactID = A.ContactID  --Left join because set to 0 in first select above
UNION
SELECT
 'DefaultAddress' [FieldType], i.PortfolioID, i.ContactID [ContactID], a.FieldName, COALESCE(a.OldValue,'') OldValue, COALESCE(a.NewValue,'') NewValue, a.AuditEventID AuditEventID
 , c.ContactCode, p.PortfolioBaseCode
 ,a.AuditEventTime [AuditEventTime]
 , '' [UserBaseName]
 , ''[FunctionDisplayName], a.UserName [AuditUserName]
  FROM AdvApp.vPortfolioInterestedParty i   ---This join makes this not work on PortfolioBases!!!
JOIN ApxUserCustom.tAC4880SA_Changes_ContactDefaultAddressFields a on a.ContactID = i.ContactID
LEFT JOIN AdvApp.vPortfolioBase p on p.PortfolioBaseID = i.PortfolioID  --Doing a left join because maybe some portfolios get deleted?
LEFT JOIN AdvApp.vContact c on c.ContactID = i.ContactID  --Left join because set to 0 in first select above
UNION
SELECT
 'Portfolio' [FieldType], a.PortfolioBaseID, 0 [ContactID], a.FieldName, COALESCE(a.OldValue,'') OldValue, COALESCE(a.NewValue,'') NewValue, a.AuditEventID AuditEventID
 , 'Portfolio' ContactCode, p.PortfolioBaseCode
 , a.AuditEventTime  [AuditEventTime]
 , '' [UserBaseName]
 , '' [FunctionDisplayName], a.UserName [AuditUserName]
...
UNION
SELECT
 'InterestedPartyMailing' [FieldType], a.PortfolioBaseID, a.ContactID [ContactID], a.FieldName, COALESCE(a.OldValue,'') OldValue, COALESCE(a.NewValue,'') NewValue, a.AuditEventID AuditEventID
 , c.ContactCode, p.PortfolioBaseCode
 ,a.AuditEventTime [AuditEventTime]
 , '' [UserBaseName]
 , ''[FunctionDisplayName], a.UserName [AuditUserName]
