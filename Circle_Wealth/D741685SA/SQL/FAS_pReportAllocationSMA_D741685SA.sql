...
/****** Object:  StoredProcedure [APXUserCustom].[FAS_pReportAllocationSMA_D741685SA]    Script Date: 03/12/2015 14:41:10 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[APXUserCustom].[FAS_pReportAllocationSMA_D741685SA]') AND type in (N'P', N'PC'))
DROP PROCEDURE [APXUserCustom].[FAS_pReportAllocationSMA_D741685SA]
GO
...
exec [APXUserCustom].[FAS_pReportAllocationSMA_D741685SA]
-- Required Parameters
...
CREATE procedure [APXUserCustom].[FAS_pReportAllocationSMA_D741685SA]
	-- Required Parameters
...
FROM (SELECT
		...
		AccruedInterest = sum(h.AccruedInterest),
		AccruedInterestIsVisible = max(convert(int,h.AccruedInterestIsVisible)),
		...
		EffectivePrice = max(h.EffectivePrice),
		EffectiveUnitCost = avg(h.EffectiveUnitCost),
	...
	FROM (		SELECT 
				...
				a.EffectivePrice,
				[EffectiveUnitCost] = case when a.SecurityTypeCode = 'peus' then NULL else a.EffectiveUnitCost end,
			...
			FROM
			(	select  
					MarketValue = isnull(a.MarketValue,0) ,
					...
					from APXUser.fAppraisal(@ReportData) a
							...
							where (select IsthisaccountaSMA from apxuser.vPortfolioBaseCustom where PortfolioBaseID=a.PortfolioBaseID)=0
					
			  union all
			  
				select 
					MarketValue = SUM(isnull(sma.MarketValue,0)),
					...
					ClassificationMemberCode = null,--classification.KeyString,
					ClassificationMemberName = MAX(COALESCE(cm2.value, @NC_Desc)),
					ClassificationMemberOrder = MAX(COALESCE(cm.DisplayOrder, @Undefined)),
					sma.PortfolioBaseID,
					sma.PortfolioBaseIDOrder,
					AccruedInterest = sum(sma.AccruedInterest),
					...
					AccruedInterestIsVisible = convert(bit, case
						when s.IsBond = 1 then (case when APXUser.fShowAccruedInterestOnAllReports(@OverridePortfolioSettings, @AccruedInterestID, (select max(AccruedInterestID) from APXUser.vPortfolioSettingEx p where p.PortfolioID = sma.PortfolioBaseID)) = 1 then 1 else 0 end)
						else 0 end),
					EffecctiveMaturityDate = null,-- a.EffectiveMaturityDate,
					EstMaturityDate = null,--a.EstMaturityDate,
					FormatPerUnit = '#,0.00',
					FormatLocalCurrency = '#,0',
					FormatQuantity = '#,0.000',--case when s.QuantityPrecision = 0 then '#,0' else '#,0.' + REPLICATE('0', s.QuantityPrecision) end,
					...
					from APXUser.fAppraisal(@ReportData) sma
							...
							LEFT JOIN apxuser.vPortfolioBaseLabels cm2 on
								sma.PortfolioBaseID = cm2.PortfolioBaseID and 
								cm2.Label='$cldef00'
							LEFT JOIN AdvApp.vSecClassMember cm on
							...
							join APXUser.vPortfolioBaseLabels l on
								sma.PortfolioBaseID = l.PortfolioBaseID
								and l.Label = '$issma'
				where l.Value=1	--and (APX.fPortfolioCustomLabel(sma.PortfolioBaseID,'$issma','0')='1')
		...
		--h.AccruedInterest,h.AccruedInterestIsVisible,
		h.IsUnsupervised,h.IsZeroMarketValue,
